...
var logger = import_winston.default.createLogger({
  ...
  transports: [
    ...
    new import_winston.default.transports.File({ filename: "system-events.log" })
  ]
});
function generateAnamnesePDF(patient, triage) {
  return new Promise((resolve, reject) => {
    ...
    doc.moveDown(0.5);
    doc.fillColor(lightText).font("Helvetica").fontSize(9).text(`Erstellungsdatum: ${(/* @__PURE__ */ new Date()).toLocaleString("de-DE")}`, { align: "center" });
    doc.moveDown(1.5);
    ...
    doc.text(`Kostentr\xE4gerkennung (IK): ${patient.ik_number || "-"}`, 65, currentY + 70);
    doc.text(`Statusfeld: ${patient.status_field || "-"}`, 300, currentY + 70);
    doc.y = currentY + 115;
    doc.moveDown(1.5);
    doc.fillColor(primaryColor).font("Helvetica-Bold").fontSize(12).text("Ersteinsch\xE4tzungs-Ergebnisse (SmED)", { underline: false });
    doc.moveDown(0.8);
    doc.fillColor(textColor).fontSize(10);
    doc.font("Helvetica-Bold").text("Hauptbeschwerde / Symptom-Kategorie:");
    doc.font("Helvetica").text(`\u2022 ${triage.symptoms || "-"}`);
    doc.moveDown(0.8);
    doc.font("Helvetica-Bold").text("Warnhinweise (Red Flags) gepr\xFCft:");
    doc.font("Helvetica").text(`\u2022 Keine akute Vitalgef\xE4hrdung gemeldet (Frage 2 negativ)`);
    doc.moveDown(0.8);
    ...
    doc.text("Der Patient hat am Kiosk-Terminal aktiv per Klick best\xE4tigt, dass das System keine medizinische Diagnose durchf\xFChrt und keinen Arztbesuch in Notf\xE4llen ersetzt. Der Patient wurde angewiesen, bei akuter Lebensgefahr die 112 zu w\xE4hlen.");
    doc.moveDown(2);
    doc.strokeColor(borderColor).lineWidth(1).moveTo(50, doc.y).lineTo(550, doc.y).stroke();
    doc.moveDown(1.5);
    doc.fillColor(lightText).font("Helvetica").fontSize(8).text("Dieses Dokument wurde im Kiosk-Modus der Apotheke automatisiert generiert und verschl\xFCsselt in der elektronischen Dokumentenablage gespeichert. Es dient ausschlie\xDFlich der Unterst\xFCtzung der \xE4rztlichen Befundung.", { align: "center", lineGap: 2 });
    doc.end();
  });
}
async function startServer() {
  ...
  const supabaseAdmin = (0, import_supabase_js.createClient)(supabaseUrl, supabaseServiceKey || "dummy-key");
  ...
  app.post("/api/kiosk/consent", async (req, res) => {
    ...
    try {
      ...
      const { data, error } = await supabaseAdmin.from("consent_agreements").insert([{
        pharmacy_id,
        patient_name,
        health_insurance_name,
        // Wichtig für den NOT NULL Constraint
  ...
  app.post("/api/kiosk/billing", async (req, res) => {
    ...
    try {
      if (!consent_id || !service_type) {
        return res.status(400).json({ error: "Fehlende Pflichtfelder f\xFCr Abrechnung" });
      }
      const { data, error } = await supabaseAdmin.from("billing_records").insert([{
        pharmacy_id,
        consent_id,
        service_type,
        amount,
  ...
  app.post("/api/kiosk/telemetry", async (req, res) => {
    try {
      ...
      const { error } = await supabaseAdmin.from("session_telemetry").insert([{
        session_id,
        pharmacy_id,
        event_type
      }]);
  ...
  app.post("/api/kiosk/generate-report", async (req, res) => {
    try {
      ...
      const { data: patient, error: patientErr } = await supabaseAdmin.from("consent_agreements").select("*").eq("id", consent_id).single();
      if (patientErr || !patient) {
        logger.error("Error fetching patient for report:", patientErr);
        return res.status(404).json({ error: "Einverst\xE4ndniserkl\xE4rung nicht gefunden" });
      }
      ...
      const { error: uploadErr } = await supabaseAdmin.storage.from("clinical-reports").upload(filePath, pdfBuffer, {
      ...
      if (billing_id) {
        const { error: updateErr } = await supabaseAdmin.from("billing_records").update({ report_path: filePath }).eq("id", billing_id);
        if (updateErr) {
  ...
  app.post("/api/billing/generate-summary", async (req, res) => {
    try {
      ...
      const targetId = appointment_id || consent_id;
      ...
      const { data: role, error: roleError } = await supabaseAdmin.from("user_pharmacy_roles").select("*").eq("user_id", user.id).eq("pharmacy_id", pharmacy_id).single();
      if (roleError || !role) {
        return res.status(403).json({ error: "Forbidden - Not authorized for this pharmacy" });
      }
      const { data: patient, error: patientErr } = await supabaseAdmin.from("consent_agreements").select("*").eq("id", targetId).single();
      if (patientErr || !patient) {
        return res.status(404).json({ error: "Patientendaten/Einverst\xE4ndnis nicht gefunden" });
      }
      const pdfBuffer = await generateAnamnesePDF(patient, triage_data || {});
      ...
      const { error: uploadErr } = await supabaseAdmin.storage.from("billing_documentation").upload(filePath, pdfBuffer, {
        contentType: "application/pdf",
        upsert: true
      });
      if (uploadErr) {
      ...
      await supabaseAdmin.from("billing_records").update({ report_path: filePath }).eq("consent_id", targetId);
      res.status(200).json({ success: true, file_path: filePath });
    } catch (error) {
  ...
  app.get("/api/admin/report-url", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.storage.from("clinical-reports").createSignedUrl(report_path, 300);
      if (error) {
  ...
  app.get("/api/admin/telemetry/stats", async (req, res) => {
    try {
      const { pharmacy_id } = req.query;
      if (!pharmacy_id) return res.status(400).json({ error: "Missing pharmacy_id" });
      const { data, error } = await supabaseAdmin.from("session_telemetry").select("*").eq("pharmacy_id", pharmacy_id);
      if (error) throw error;
      const sessionsStarted = data.filter((e) => e.event_type === "SESSION_STARTED").length;
      const sessionsCompleted = data.filter((e) => e.event_type === "SESSION_COMPLETED").length;
      const sessionsAborted = data.filter((e) => e.event_type === "SESSION_ABORTED").length;
      const successRate = sessionsStarted > 0 ? sessionsCompleted / sessionsStarted * 100 : 0;
      let dropTriage = 0, dropConsent = 0, dropService = 0;
      data.filter((e) => e.event_type === "SESSION_ABORTED").forEach((e) => {
      });
      res.json({
  ...
  app.post("/api/admin/pharmacies", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.from("pharmacies").insert([
        {
          name,
          ik_nummer,
          bsnr,
  ...
  app.post("/api/admin/terminals/pair", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.from("pairing_codes").insert([{ pharmacy_id, code, expires_at: expiresAt }]).select("code").single();
      if (error) return res.status(500).json({ error: error.message });
  ...
  app.post("/api/kiosk/authenticate", async (req, res) => {
    try {
      ...
      const { data: codeData, error: codeErr } = await supabaseAdmin.from("pairing_codes").select("*").eq("code", code).gte("expires_at", (/* @__PURE__ */ new Date()).toISOString()).single();
      if (codeErr || !codeData) {
        return res.status(400).json({ error: "Invalid or expired code" });
      }
      const deviceToken = "atm_" + Math.random().toString(36).substring(2) + Date.now().toString(36);
      const { data: terminalData, error: terminalErr } = await supabaseAdmin.from("kiosk_terminals").insert([{
  ...
  app.get("/api/admin/terminals", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.from("kiosk_terminals").select("*").eq("pharmacy_id", pharmacy_id).order("created_at", { ascending: false });
      if (error) return res.status(500).json({ error: error.message });
      res.json(data);
    } catch (error) {
      res.status(500).json({ error: error.message });
  ...
  app.delete("/api/admin/terminals/:id", async (req, res) => {
    try {
      const { error } = await supabaseAdmin.from("kiosk_terminals").update({ status: "revoked" }).eq("id", req.params.id);
      if (error) return res.status(500).json({ error: error.message });
  ...
  app.post("/api/admin/accept-avv", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.from("pharmacies").update({ avv_akzeptiert_am: timestamp }).eq("id", pharmacy_id).select("avv_akzeptiert_am").single();
      if (error) return res.status(500).json({ error: error.message });
  ...
  app.put("/api/admin/pharmacies/:id", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.from("pharmacies").update(updates).eq("id", id).select("*").single();
      if (error) {
        return res.status(500).json({ error: error.message });
      }
      res.json(data);
  ...
  app.post("/api/auth/register-b2b", async (req, res) => {
    try {
      ...
      const { data: pharmacy, error: pharmacyErr } = await supabaseAdmin.from("pharmacies").insert([
        {
          name,
          ik_nummer,
          bsnr,
  ...
  app.post("/api/pharmacy/upload-document", async (req, res) => {
    try {
      ...
      const { error: uploadErr } = await supabaseAdmin.storage.from("pharmacy-documents").upload(filePath, uint8Array, {
        contentType: "application/pdf",
        upsert: true
      });
      if (uploadErr) {
      ...
      const { data: pharmacy, error: fetchErr } = await supabaseAdmin.from("pharmacies").select("*").eq("id", pharmacy_id).single();
      ...
      const { error: updateErr } = await supabaseAdmin.from("pharmacies").update(updateFields).eq("id", pharmacy_id);
      if (updateErr) {
  ...
  app.get("/api/admin/pharmacy-document-url", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.storage.from("pharmacy-documents").createSignedUrl(report_path, 300);
      if (error) {
  ...
  app.post("/api/admin/pharmacies/:id/verify-documents", async (req, res) => {
    try {
      ...
      const { data, error } = await supabaseAdmin.from("pharmacies").update({
        onboarding_status,
  ...
  const SKZ_LABELS = {
    "19816342": "Kombileistung (Ersteinsch\xE4tzung + Video)",
    "19816313": "Nur Ersteinsch\xE4tzung",
    "19816336": "Nur Videosprechstunde"
  };
  const getBillingAggregation = async (pharmacy_id, month) => {
    const startDate = `${month}-01`;
    const nextMonth = new Date(startDate);
    nextMonth.setMonth(nextMonth.getMonth() + 1);
    const endDate = nextMonth.toISOString().split("T")[0];
    const { data: pharmacy, error: pharmErr } = await supabaseAdmin.from("pharmacies").select("name, ik_number, bsnr").eq("id", pharmacy_id).single();
    if (pharmErr) throw new Error("Pharmacy not found");
    const { data: records, error: recordsErr } = await supabaseAdmin.from("billing_records").select("*").eq("pharmacy_id", pharmacy_id).gte("date_of_service", startDate).lt("date_of_service", endDate);
    ...
    try {
      ...
      const exportData = {
        Abrechnungs_Metadaten: {
          ...
          Erstellungsdatum: (/* @__PURE__ */ new Date()).toISOString(),
...
`;
  ...
  app.get("/api/admin/billing/export-pdf/:pharmacy_id", async (req, res) => {
    try {
      ...
      doc.pipe(res);
      doc.fontSize(20).text("Sonderbeleg f\xFCr Telemedizinische Leistungen (aTM)", { align: "center" });
      doc.moveDown(2);
      ...
      doc.text(`Erstellungsdatum: ${(/* @__PURE__ */ new Date()).toLocaleDateString("de-DE")}`);
      doc.moveDown(2);
      const startY = doc.y;
      ...
      doc.moveDown(4);
      doc.font("Helvetica").fontSize(10).text("Rechtsgrundlage: \xA7 129 SGB V", 50, doc.y);
      doc.moveDown(4);
  ...
  const stripe = new import_stripe.default(process.env.STRIPE_SECRET_KEY || "sk_test_placeholder", {
  ...
  app.post("/api/stripe/portal-session", async (req, res) => {
    try {
      const { pharmacy_id } = req.body;
      const { data: pharmacy } = await supabaseAdmin.from("pharmacies").select("stripe_customer_id").eq("id", pharmacy_id).single();
    ...
    try {
      switch (event.type) {
        ...
        case "customer.subscription.deleted": {
          ...
          const { data: sub } = await supabaseAdmin.from("subscriptions").select("pharmacy_id").eq("stripe_subscription_id", subscription.id).single();
          if (sub?.pharmacy_id) {
            await supabaseAdmin.from("pharmacies").update({ subscription_status: subscription.status }).eq("id", sub.pharmacy_id);
          }
          break;
        }
      }
  ...
  app.post("/api/chat", async (req, res) => {
    try {
      ...
      const chat = genAI.chats.create({
        ...
        // using flash for higher quota limits and speed
        config: {
          systemInstruction: "Du bist ein hilfreicher und professioneller Assistent f\xFCr eine deutsche Apotheke im Bereich der Assistierten Telemedizin (aTM). Du hilfst Apothekern bei Fragen zu Ersteinsch\xE4tzung, Videosprechstunden und Sonderkennzeichen. Antworte immer auf Deutsch und professionell."
        }
      });
      ...
      const response = await genAI.models.generateContent({
        ...
        contents: {
          parts: [
            ...
            {
              text: prompt || "Bitte analysiere dieses Bild und fasse zusammen, was du siehst. Ist es f\xFCr die Telemedizin relevant?"
            }
          ]
        },
        config: {
          systemInstruction: "Du bist ein hilfreicher und professioneller Assistent f\xFCr eine deutsche Apotheke im Bereich der Assistierten Telemedizin (aTM). Antworte auf Deutsch."
        }
      });
      res.json({ text: response.text });
    } catch (error) {
  ...
  app.get("/api/admin/audit-log/:pharmacy_id", async (req, res) => {
    try {
      const { pharmacy_id } = req.params;
      const { data: pharmacy, error: pharmErr } = await supabaseAdmin.from("pharmacies").select("*").eq("id", pharmacy_id).single();
      ...
      doc.text(`IK-Nummer: ${pharmacy.ik_number || "Nicht hinterlegt"}`);
      doc.text(`BSNR: ${pharmacy.bsnr || "Nicht hinterlegt"}`);
      doc.text(`Datum der Erstellung: ${(/* @__PURE__ */ new Date()).toLocaleDateString("de-DE")}`);
      const avvDate = new Date(pharmacy.avv_akzeptiert_am).toLocaleString("de-DE");
      doc.text(`AVV digital akzeptiert am: ${avvDate}`);
      doc.moveDown();
      doc.font("Helvetica-Bold").text("2. Beschreibung der Verarbeitungst\xE4tigkeiten");
      doc.font("Helvetica").text("Zweck: Durchf\xFChrung von Ersteinsch\xE4tzungen und Videosprechstunden im Rahmen der assistierten Telemedizin gem\xE4\xDF \xA7 129 SGB V.");
      ...
      doc.moveDown();
      doc.font("Helvetica-Bold").text("5. Technische und organisatorische Ma\xDFnahmen (TOMs)");
      doc.font("Helvetica").text("- Die Videosprechstunde erfolgt \xFCber eine Ende-zu-Ende verschl\xFCsselte WebRTC/Jitsi-Infrastruktur.");
      doc.text("- Es erfolgt KEINE serverseitige Aufzeichnung der Videostreams.");
      doc.text("- Zugriff auf das Apotheken-Dashboard ist durch rollenbasierte Autorisierung (Supabase Auth) und RLS (Row Level Security) gesch\xFCtzt.");
      doc.moveDown();
      doc.font("Helvetica-Bold").text("6. L\xF6schfristen");
      doc.font("Helvetica").text("Einverst\xE4ndniserkl\xE4rungen (AVV) sowie Abrechnungsdaten werden gem\xE4\xDF gesetzlicher Vorgaben f\xFCr 4 Jahre (retention_expires_at) verschl\xFCsselt aufbewahrt und danach automatisiert vernichtet.");
      doc.moveDown(2);
      doc.fontSize(10).fillColor("gray").text("Dieses Dokument wurde maschinell generiert und entspricht den Anforderungen der Landesapothekerkammern (LAK) f\xFCr die digitale Vor-Ort-Versorgung.", { align: "center" });
      doc.end();
    } catch (error) {
